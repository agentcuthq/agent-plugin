...
`import_session` also returns the empty project's browser handoff. **Open it
immediately and make the in-app Browser visible before running the helper.**
Opening or focusing the tab is enough; do not wait for the project page to
finish rendering. Follow the host-basics skill for tab reuse, credential
secrecy, deferred Browser discovery, origin approval, and safe fallback.
...
For each video you are about to work on, the next MCP call after the helper is
**`get_video_url {video_id}`**. Make that call before `run_status`, transcript
polling, or any other long work, and navigate the existing project tab to its
authenticated handoff URL. Do not open one tab per imported file: in a batch,
keep one AgentCut tab aligned to the video currently being edited and retarget
it when you switch videos.
...
- **`video_id` — immediately.** As soon as it appears in the JSON you can
  address the video with every video-scoped tool. Call `get_video_url` now to
  retarget the visible project tab; show the user only the clean `web_url`.
