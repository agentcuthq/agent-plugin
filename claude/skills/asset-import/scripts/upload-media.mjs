...
function webVideoUrl(base, _projectId, videoId, env = process.env) {
  const explicit = firstNonEmpty(env.AGENTCUT_APP_BASE);
  let appBase = explicit ? stripTrailingSlash(explicit) : stripTrailingSlash(base);
  try {
    const api = new URL(appBase);
    if (!explicit && api.origin === DEFAULT_API_BASE) appBase = DEFAULT_APP_BASE;
    if (!explicit && api.hostname === "localhost" && api.port === "3000") {
      api.port = "5173";
      appBase = api.origin;
    }
  } catch {
  }
  return `${appBase}/videos/${encodeURIComponent(videoId)}`;
}
...
function timelineCheckUrl(base, videoId) {
  return videoUrl(base, videoId, "timeline/check");
}
function timelineCommitUrl(base, videoId) {
  return videoUrl(base, videoId, "timeline/commit");
}
function reviewCommentsUrl(base, videoId, action) {
  return videoUrl(base, videoId, `review-comments/${action}`);
}
...
function parseRenderCreate(data) {
  ...
  return {
    ...
    planSha256: typeof o.plan_sha256 === "string" ? o.plan_sha256 : null
  };
}
...
function parseFileList(data) {
  ...
  return arr.filter((f) => f !== null && typeof f === "object").map((f) => ({
    ...
    revision: Number.isInteger(f.revision) ? Number(f.revision) : 0
  }));
}
...
function parseFileContent(data) {
  ...
  return {
    ...
    ...Number.isInteger(o.base_revision) ? { baseRevision: Number(o.base_revision) } : {},
    ...typeof o.compile_context_sha256 === "string" ? { compileContextSha256: o.compile_context_sha256 } : {}
  };
}
...
var DEFAULT_TIMEOUT_MS, DEFAULT_DOWNLOAD_TIMEOUT_MS, DEFAULT_API_BASE, DEFAULT_APP_BASE, CloudHttpError, RETRYABLE_STATUSES, RequestTimeoutError, REQUEST_MAX_ATTEMPTS, CloudClient;
var init_client = __esm({
  "src/cloud/client.ts"() {
    ...
    DEFAULT_API_BASE = "https://api.agentcut.io";
    DEFAULT_APP_BASE = "https://agentcut.io";
    ...
    CloudClient = class _CloudClient {
      ...
      /** Candidate validation deliberately returns 409/422 domain verdicts as
       * data. Only auth, transport, and unexpected server failures throw. */
      async timelineVerdict(url, body) {
        try {
          return asRecord(await this.request("POST", url, body), "timeline verdict");
        } catch (error) {
          if (error instanceof CloudHttpError && (error.status === 409 || error.status === 422) && error.body !== null && typeof error.body === "object") {
            return error.body;
          }
          throw error;
        }
      ...
      // ---- render -------------------------------------------------------------
      async createRender(mode, planSha256, approvedPreviewRunId) {
        const data = await this.request("POST", rendersUrl(this.cfg.base), {
          video_id: this.requireVideoId(),
          mode,
          ...planSha256 ? { plan_sha256: planSha256 } : {},
          ...approvedPreviewRunId ? { approved_preview_run_id: approvedPreviewRunId } : {}
        });
      ...
      async qaFrames(args) {
        ...
        if (args.candidate !== void 0) body.candidate = args.candidate;
      ...
      async qaWaveform(args) {
        const data = await this.request("POST", qaUrl(this.cfg.base, this.requireVideoId()), {
          ...
          ...args.candidate !== void 0 ? { candidate: args.candidate } : {}
        });
      ...
      async timelineBase() {
        const data = asRecord(
          await this.request("GET", filesUrl(this.cfg.base, this.requireVideoId())),
          "timeline base"
        );
        const value = asRecord(data.timeline, "timeline base metadata");
        if (!Number.isInteger(value.base_revision) || typeof value.compile_context_sha256 !== "string") {
          throw new Error(`timeline base metadata is incomplete: ${JSON.stringify(data.timeline)}`);
        }
        return {
          baseRevision: Number(value.base_revision),
          compileContextSha256: value.compile_context_sha256,
          timelineSha256: typeof value.timeline_sha256 === "string" ? value.timeline_sha256 : null,
          compiledPlanRevision: Number.isInteger(value.compiled_plan_revision) ? Number(value.compiled_plan_revision) : null,
          compiledPlanSha256: typeof value.compiled_plan_sha256 === "string" ? value.compiled_plan_sha256 : null,
          diagnosticsRevision: Number.isInteger(value.diagnostics_revision) ? Number(value.diagnostics_revision) : null,
          diagnosticsSha256: typeof value.diagnostics_sha256 === "string" ? value.diagnostics_sha256 : null
        };
      }
      async filesGet(path) {
      ...
      async timelineCheck(body) {
        return this.timelineVerdict(timelineCheckUrl(this.cfg.base, this.requireVideoId()), body);
      }
      async timelineCommit(body) {
        return this.timelineVerdict(timelineCommitUrl(this.cfg.base, this.requireVideoId()), body);
      }
      async reviewCommentsInbox() {
        return asRecord(
          await this.request("GET", reviewCommentsUrl(this.cfg.base, this.requireVideoId(), "inbox")),
          "review inbox"
        );
      }
      async reviewCommentsPull(body = {}) {
        return asRecord(
          await this.request("POST", reviewCommentsUrl(this.cfg.base, this.requireVideoId(), "pull"), body),
          "review pull"
        );
      }
      async reviewCommentsResolve(body) {
        return asRecord(
          await this.request("POST", reviewCommentsUrl(this.cfg.base, this.requireVideoId(), "resolve"), body),
          "review resolve"
        );
      }
...
import { existsSync, mkdirSync as mkdirSync2, readFileSync, renameSync, writeFileSync as writeFileSync2 } from "fs";
...
function writeJobFile(jobDir, job) {
  ...
  writeFileSync2(temporary, JSON.stringify(job, null, 2) + "\n", "utf8");
...
function readJobFile(jobDir) {
  ...
  try {
    ...
    return {
      ...
      ...Number.isInteger(raw.timeline_revision) && Number(raw.timeline_revision) >= 0 ? { timeline_revision: Number(raw.timeline_revision) } : {},
      ...typeof raw.timeline_sha256 === "string" ? { timeline_sha256: raw.timeline_sha256 } : {},
      ...Number.isInteger(raw.compiled_plan_revision) && Number(raw.compiled_plan_revision) >= 0 ? { compiled_plan_revision: Number(raw.compiled_plan_revision) } : {},
      ...typeof raw.compiled_plan_sha256 === "string" ? { compiled_plan_sha256: raw.compiled_plan_sha256 } : {},
      ...typeof raw.compile_context_sha256 === "string" ? { compile_context_sha256: raw.compile_context_sha256 } : {},
...
function isMultipartKindUnsupported(err, kind) {
  if (!(err instanceof CloudHttpError) || err.status !== 400) return false;
  const body = err.body;
  const error = typeof body === "object" && body !== null ? body.error : void 0;
  return error?.code === "invalid_request" && typeof error.message === "string" && error.message.includes("kind must be") && !error.message.includes(`"${kind}"`);
}
...
async function uploadMultipart(client, opts, timeoutMs = DEFAULT_DOWNLOAD_TIMEOUT_MS) {
  ...
  if (opts.renderId !== void 0) createBody.render_id = opts.renderId;
  let session;
  try {
    ...
    if (isMultipartUnavailable(err) || isMultipartKindUnsupported(err, opts.kind)) return "unavailable";
  ...
  "src/cloud/runs.ts"() {
    ...
    RunPollTimeoutError = class extends Error {
      constructor(runId, lastStatus, elapsedMs) {
        super(
          `stopped waiting on run ${runId} after ${Math.round(elapsedMs / 1e3)}s (status: ${lastStatus}); the run may still finish server-side \u2014 check it with \`agentcut status ${runId}\``
        );
        this.runId = runId;
...
async function readProbe(path, ffprobe, run) {
...
async function probeMedia(path, ffprobe, run = runProcess) {
  const raw = await readProbe(path, ffprobe, run);
...
function parseImportArgs(argv, env = process.env) {
  ...
  if (!options.sessionToken || !options.endpoint) {
    throw new Error(
      "--token and --endpoint are mandatory; create an import session first with the AgentCut `import_session` tool"
    );
  }
...
Options:
  ...
  --transcribe-wait-ms <ms>  Cap on waiting for the transcribe run (default:
                         ...
                         import returns with transcript "started" \u2014 poll its
                         run id with AgentCut MCP \`run_status\`.
...
`
  ...
  if (!tools) {
    progress(
      "no usable ffmpeg/ffprobe found (ffprobe must support stream_side_data); use the cloud import route: MCP upload_start \u2192 video_create \u2192 ingest_start"
    );
    process.stderr.write(
      `${JSON.stringify({
        ok: false,
        message: "local media tools unavailable; use the cloud import route: MCP upload_start \u2192 video_create \u2192 ingest_start"
      })}
`
    );
    return 1;
